...
/**
 * A Grid object holds the currently known values of the Sudoku puzzle. The grid
 * consists of 9x9 cells that hold integer values from 0 to 9. A value of 0
 ...
 * locations are ordered from left-to-right and top-down. For example, location
 * 0 refers to the top-leftmost cell. Location 8 refers to the top-righttmost
 * cell. Location 71 refers to the bottom-leftmost cell. Location 80 refers to
 ...
 * subgrid contains a subgrid of 3x3 cells. Subgrid 0 contains cells - 0, 1, 2,
 * 9, 10, 11, 18, 19, 20. Subgrid 8 contains cells - 60, 61, 62, 69, 70, 71, 78,
 * 79, 80
 */
public class Grid implements Cloneable {
	 ...
	 * The cells containing givens have values between 1 and 9. Empty cells have
	 * the value 0.
	 * 
	 * Characters are read one at a time from the input stream and placed into
	 * the grid in left-to-right and top-down order. - The characters 0 or .
	 * indicates an empty cell. - The characters 1 to 9 indicates a given. - The
	 * character # is used for comments; subsequent characters are ignored until
	 * a newline is encountered. - All other characters are simply ignored.
	 * 
	 * @param rd
	 ...
	 * empty 2. the cell is allowed to contain the specified number. E.g. if the
	 * number is 5 but the row already has a 5, the cell will not be set and
	 * false is returned.
	 * 
	 * @param loc the location of the target cell. Values must be in the range
	 * [0, 80].
	 * 
	 * @param num the number to set in the cell. Values must be in the range [1,
	 * 9].
	 * 
	...
	public boolean set(int loc, int num) {
		...
		boolean canSet = cells[loc] == 0 && (colsSet[c] & (1 << num)) == 0 && (rowsSet[r] & (1 << num)) == 0
	...
	/**
	 * Returns a copy of this grid. Any modifications to the returned grid will
	 * not affect this grid.
	 * 
	...
	/**
	 * Returns a string representing the current contents of the grid. Used for
	 * debugging purposes.
	 * 
